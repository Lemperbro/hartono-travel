...
<div id="dropdown{{ $item->id }}"
    ...
    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
        <li>
            <a href="{{ route('kapal.edit', ['id' => $item->slug]) }}"
                class="block px-4 py-2 hover:bg-gray-100 w-full text-Sidebar">Edit Data</a>
        </li>
        <li>
            <form action="{{ route('kapal.set.expired', ['id' => $item->slug]) }}" method="POST">
                @csrf
                <button type="submit" class="block px-4 py-2 hover:bg-gray-100 w-full text-left text-yellow-600">Tiket
                    Habis</button>
            </form>
        </li>
        <li>
            <form action="{{ route('kapal.delete', ['id' => $item->slug]) }}" method="POST">
                @csrf
