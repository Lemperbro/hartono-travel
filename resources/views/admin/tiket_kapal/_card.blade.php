...
<div
    ...
    <img class="object-cover w-full rounded-t-lg h-52 lg:h-full lg:w-48 lg:rounded-none lg:rounded-s-lg"
        src="{{ asset('TiketKapalImage/'.$item->image) }}" alt="">
    <div class="flex flex-col justify-between p-4 leading-normal w-full">
        ...
        <h1 class="mt-2 text-sm text-red-600">Tiket Habis</h1>
        @endif
        <h5 class="mb-2 text-xl lg:text-2xl font-bold tracking-tight text-Sidebar">{{ $item->title }}</h5>
        <div class="flex flex-col gap-4">
            <div>
                <h1 class="text-Sidebar font-semibold">Keberangkatan</h1>
                <h3 class="text-sm text-gray-600">{{ $item->keberangkatan }}</h3>
                <h3 class="text-sm text-gray-600">{{ Carbon::parse($item->waktu_keberangkatan)->locale('id')->isoFormat('dddd, DD MMMM YYYY') }}</h3>
            </div>
            <div>
                <h1 class="text-Sidebar font-semibold">Kedatangan</h1>
                <h3 class="text-sm text-gray-600">{{ $item->tujuan }}</h3>
                <h3 class="text-sm text-gray-600">{{ Carbon::parse($item->waktu_kedatangan)->locale('id')->isoFormat('dddd, DD MMMM YYYY') }}</h3>
            </div>
        </div>
    </div>
</div>
