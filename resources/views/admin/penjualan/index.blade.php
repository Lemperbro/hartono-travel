        ...
        <div class="pt-24">
            ...
            <div class="p-4 grid grid-cols-1 bg-white border-[1px] border-main3 rounded-md mt-4">
                ...
                <form class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4"
                    action="{{ route('penjualan') }}" method="GET">
                    <div>
                        ...
                        <select name="bulan" id="bulan"
                            ...
                            @foreach ($bulan as $item)
                                <option value="{{ $loop->iteration }}"
                                    {{ $loop->iteration == request('bulan') ? 'selected' : '' }}>
                                    {{ $item }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="sudah bayar" {{ request('status') == 'sudah bayar' ? 'selected' : '' }}>Sudah
                                Dibayar</option>
            ...
            <div class="p-4 flex flex-wrap gap-4  bg-white border-[1px] border-main3 rounded-md mt-4">
                <a href="{{ route('penjualan.create') }}"
                    class="bg-Sidebar py-2 px-4 rounded-md text-white flex gap-2 items-center font-medium">
                    ...
                    Tambah Data</a>
                <a href="{{ route('penjualan', array_merge(request()->all(), ['download' => true])) }}"
                    class="bg-green-700 py-2 px-4 rounded-md text-white flex gap-2 items-center font-medium">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                        class="fill-white w-7 h-7">
                        <path
